...
#Region " Windows Form Designer generated code "
        ...
        Public Sub New()
            ...
            If UBound(Diagnostics.Process.GetProcessesByName(Process.GetCurrentProcess.ProcessName)) > 0 Then
                MsgBox("Already CXL Robot(Single DB) instance is running")
                End
            End If
        ...
        <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
            ...
            Me.Exception.Size = New System.Drawing.Size(100, 23)
            ...
            'BtnExit
            '
            Me.BtnExit.Location = New System.Drawing.Point(16, 152)
            ...
            Me.Text = "CXL ROBOT 2.0 (SINGLE DB) 05.11.10"
...
#Region "..................... Public Variable................................"
        ...
        Public obj As CXL.CXLTRANSACTION = New CXL.CXLTRANSACTION

        ' This is Serviceactivation for InfiniReSellers' orders

        Public ObjIo As Io.InfiniShops.ServiceActivation = New Io.InfiniShops.ServiceActivation

        'This is ServiceActivation for AccountsCentres' Orders
        Public ObjService As ServiceActivation.ServiceActivation = New ServiceActivation.ServiceActivation
        ...
        Public ObjBuyer As New BPtsPayment.InfiniBPPay

        'This is ServiceActivation for InfiniMerkets' Orders
        <CLSCompliant(False)> Public ObjInifiniMarketPlace As New InfiniMarket._Default

        'This is For Paypal
        Public objPayPal As InfiniPaypal.InfiniPayPalWebService = New InfiniPaypal.InfiniPayPalWebService
...
#Region ".............................Private Variables......................."
        ...
        Dim OrderDataExists As Boolean = False
        Dim CustomerIdMissing As String = "CustomerId Not Found"
#End Region
...
#End Region
        ...
        ' to be used in Framework 2.0 against CertificateValidation
        Private Function TrustAllCertificatesCallback(ByVal sender As Object, ByVal cert As X509Certificate, ByVal chain As X509Chain, ByVal errors As System.Net.Security.SslPolicyErrors) As Boolean
            'This function handles problems with certificates if ssl (https) is used
            Return True
        End Function
        ...
        Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            'System.Net.ServicePointManager.CertificatePolicy = New CertificateValidation ' to be used in Framework 1.1
            System.Net.ServicePointManager.ServerCertificateValidationCallback = New System.Net.Security.RemoteCertificateValidationCallback(AddressOf TrustAllCertificatesCallback)

            'IDENTITY = New ArrayList

            'IDENTITY.Add("DATACENTRE2")

            '////////Begin Modification as per RI instructions 19112015

            Dim DataCentres As String = System.Configuration.ConfigurationManager.AppSettings("DATACENTRES")
            ...
            Next
            '////////End  Modification as per RI instructions 19112015
            ...
            Try
                ...
                If (mysqlds.Tables(0).Rows.Count > 0) Then
                    ...
                    For i = 0 To mysqlds.Tables(0).Rows.Count - 1
                        ...
                        Try
                            obj.InsertMySqlDataInSqlServer(loginid, orderbooked, Amount, requesttime, Timestamp1, status, cardno, cardtype, cardname, cardaddress, CardExpir, securitycode, issueno, processtype, genericcode, currencycode, CardStar, trackid_atcs, CreateInvoice, nocxl, authorize)

                        Catch ex As Exception
                            LstException.Items.Add(DateTime.Now & " " & ex.Message & " insert my sql data in sql server" & obj.GetExcept)

                            If Not UCase(System.Configuration.ConfigurationManager.AppSettings("emailFrom")) = "LOCAL" Then
...
#End Region
        ...
        Private Sub BindGrid()
            Try
                ' Encryption of Cardno, issue number and security codel
            ...
            Try
                ...
                If (Ds.Tables(0).Rows.Count > 0) Then
                    ...
                    For i = 0 To Ds.Tables(0).Rows.Count - 1
                        ...
                        If UCase(System.Configuration.ConfigurationManager.AppSettings("ThreeDSecure")) = "ON" Then
                            Try
                            ...
                            Catch ex As Exception
                                ...
                                LstException.Items.Add(DateTime.Now & " " & "Checking validity 3D Secure;" & ex.Message)
                            End Try
                        End If
                        ...
                        If ChkLog.Checked = False Then
                            ...
                            ElseIf (OrderID <> "0") Then
                                filename = "Order - " & OrderID & "-" & MerchantId
                                filename = folder & "\" & filename
                            End If

                            log = ":: CXL SDB, ver:151209"
                        ...
                        If (CSAvailable <> 0) Then
                            ...
                            If (Ds.Tables(0).Rows(i).Item("mode") = "Y") Or CreditCardNo = "4444333322221111" Then
                                Mode = "TEST"
                            Else
                                Mode = "LIVE"
                            End If
                            ...
                            If ((UCase(ProcessType) = "CB") Or (UCase(ProcessType) = "CC")) And Cust_ID = 0 And OrderDataExists = False Then
                                Try
                                    SendEmail("Sender = " & Sender & " OrderID= " & OrderID & " MerchantId = " & MerchantId & " TrackId = " & rid & " Exception= " & CustomerIdMissing)
                                Catch ex As Exception
                            ...
                            'Implementing Rules For Block CountryCode, CreditCard ,CustomerBlock and IP
                            ...
                            '        For Count = 0 To ExistRule.Tables(0).Rows.Count - 1
                            ...
                            '                    For Count2 = 0 To Ds2.Tables(0).Rows.Count - 1
                            '                        If (CreditCardNo = Trim(Ds2.Tables(0).Rows(Count2).Item("Rule")) Or IPAddress = Trim(Ds2.Tables(0).Rows(Count2).Item("Rule"))) Then
                            '                            Blocked = False
                            '                            Exit For
                            '                        Else
                            '                            Blocked = True
                            ...
                            '                    If ExistRule.Tables(0).Rows(Count).Item("RuleType") = "4" Then
                            '                        'Implementing block rule which will convert 'cc' into 'cb' for specific customer
                            '                        If UCase(ProcessType) = "CC" Then
                            '                            log = "Checking order for Block Rule that can change the payment mode"
                            '                            WriteDebugInfo(log, filename)

                            '                            Dim OrderInfoMerchantInvoice As DataSet = obj.GetCustomerOrderInfo(Ds.Tables(0).Rows(i).Item("CustomerID"), MerchantId, OrderID)
                            '                            If Not OrderInfoMerchantInvoice Is Nothing Then
                            '                                If CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("Decrypt_cart_invoice_cardname")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("Decrypt_cart_invoice_cardname"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("Decrypt_cart_invoice_cardaddress")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("Decrypt_cart_invoice_cardaddress"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("del_add")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("del_add"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("name")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("name"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("cont_name")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("cont_name"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("street")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("street"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Or CInt(CStr(IIf(IsDBNull(OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("town")), "", OrderInfoMerchantInvoice.Tables(1).Rows(0).Item("town"))).IndexOf(Trim(CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))))) <> -1 Then
                            '                                    log = "Changing 'CC' to 'CB' as order has been qualified for block rule where DBName = " & Dbname & "and MerchantId = " & MerchantId & "and orderID = " & OrderID & "ProcessType = " & ProcessType & "Block Keyword = " & CStr(ExistRule.Tables(0).Rows(Count).Item("Rule"))
                            '                                    WriteDebugInfo(log, filename)
                            '                                    ProcessType = "cb"
                            '                                End If
                            '                            End If
                            ...
                            If (Check = True) Then
                                ...
                                If (UCase(ProcessType) = "CC") And OrderDataExists = True Then
                                ...
                                ElseIf (UCase(ProcessType) = "CB") And OrderDataExists = True OrElse (UCase(ProcessType) = "CD") Then
                            ...
                            Else
                                ...
                                If ChkLog.Checked = False Then
                                    ...
                                    ElseIf (OrderID <> "0") Then
                                        ...
                                        WriteDebugInfo(log, filename)
                                    End If
                                    If Cust_ID = 0 And OrderDataExists = False Then
                                        log = ". " & CustomerIdMissing
                                        WriteDebugInfo(log, filename)
                                    End If
                                ...
                                DataError(7) = IIf(Cust_ID = 0 And OrderDataExists = False, CustomerIdMissing, CsMessage)
                    ...
                    Try
                        ...
                        ElseIf (NormalizeData.Tables(0).Rows(L).Item("sender") = "IR" Or NormalizeData.Tables(0).Rows(L).Item("sender") = "IN") Then
                            ...
                            If ChkLog.Checked = False Then
                                ...
                                log = log & ". @InvoiceStatus= " & "D" & vbCrLf
                                log = log & ". @CXLCode= " & 500 & vbCrLf
                                log = log & ". @CXLMessage= " & IIf(Cust_ID = 0, CustomerIdMissing, "NULL") & vbCrLf
                                log = log & ". @ProcessType= " & NormalizeData.Tables(0).Rows(L).Item("ProcessType") & vbCrLf
                                log = log & ". @OrderID= " & NormalizeData.Tables(0).Rows(L).Item("OrderID") & vbCrLf
                            ...
                            If Cust_ID = 0 And OrderDataExists = False Then
                                'Dim CustomerIdMissing As String = "CustomerId Not Found"
                                obj.UpdateInvoiceStatus(NormalizeData.Tables(0).Rows(L).Item("MerchantID"), NormalizeData.Tables(0).Rows(L).Item("TrackID"), "D", 500, IIf(Cust_ID = 0, CustomerIdMissing, "NULL"), NormalizeData.Tables(0).Rows(L).Item("ProcessType"), NormalizeData.Tables(0).Rows(L).Item("OrderID"))
                                obj.UpdateInvoiceTable(NormalizeData.Tables(0).Rows(L).Item("MerchantId"), NormalizeData.Tables(0).Rows(L).Item("OrderID"), IIf(Cust_ID = 0, CustomerIdMissing, "NULL"), "500", NormalizeData.Tables(0).Rows(L).Item("trackid"), NormalizeData.Tables(0).Rows(L).Item("sender"), "D")
                            Else
                                obj.UpdateInvoiceStatus(NormalizeData.Tables(0).Rows(L).Item("MerchantID"), NormalizeData.Tables(0).Rows(L).Item("TrackID"), "D", 500, "NULL", NormalizeData.Tables(0).Rows(L).Item("ProcessType"), NormalizeData.Tables(0).Rows(L).Item("OrderID"))
                                obj.UpdateInvoiceTable(NormalizeData.Tables(0).Rows(L).Item("MerchantId"), NormalizeData.Tables(0).Rows(L).Item("OrderID"), "NULL", "500", NormalizeData.Tables(0).Rows(L).Item("trackid"), NormalizeData.Tables(0).Rows(L).Item("sender"), "D")
                            ...
                            If (NormalizeData.Tables(0).Rows(L).Item("sender") = "IR") Then
                                ...
                                Try
                                    ...
                                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(0).Rows(L).Item("OrderID"), NormalizeData.Tables(0).Rows(L).Item("MerchantID"), NormalizeData.Tables(0).Rows(L).Item("MerchantLoginID"), NormalizeData.Tables(0).Rows(L).Item("Customer_Id"), "N", "N", PayModeMPI, "CXL_IR")
                                    If ChkLog.Checked = False Then
                            ...
                            ElseIf (NormalizeData.Tables(0).Rows(L).Item("sender") = "IN") Then
                                ...
                                Try
                                    ...
                                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(0).Rows(L).Item("OrderID"), NormalizeData.Tables(0).Rows(L).Item("MerchantID"), NormalizeData.Tables(0).Rows(L).Item("MerchantLoginID"), NormalizeData.Tables(0).Rows(L).Item("Customer_Id"), "N", "N", PayModeMPI, "CXL_IN")
                                    If ChkLog.Checked = False Then
                                        log = ". Dumping data for MPI Reseller" & vbCrLf
                                        log = log & "Exception:" & obj.GetExcept
                                        WriteDebugInfo(log, filename)
                            ...
                            End If
                            If Cust_ID = 0 And OrderDataExists = False Then
                                obj.UpdateResponse_New(NormalizeData.Tables(0).Rows(L).Item("MerchantId"), NormalizeData.Tables(0).Rows(L).Item("CLoginId"), NormalizeData.Tables(0).Rows(L).Item("OrderID"), "D", NormalizeData.Tables(0).Rows(L).Item("CsMessage"), NormalizeData.Tables(0).Rows(L).Item("ResponseCode_Cs"), 0, NormalizeData.Tables(0).Rows(L).Item("trackid"), CxlIdentity)
                            Else
                                obj.UpdateResponse_New(NormalizeData.Tables(0).Rows(L).Item("MerchantId"), NormalizeData.Tables(0).Rows(L).Item("CLoginId"), NormalizeData.Tables(0).Rows(L).Item("OrderID"), "S", NormalizeData.Tables(0).Rows(L).Item("CsMessage"), NormalizeData.Tables(0).Rows(L).Item("ResponseCode_Cs"), 0, NormalizeData.Tables(0).Rows(L).Item("trackid"), CxlIdentity)
                            End If
                        ...
                        ElseIf (NormalizeData.Tables(0).Rows(L).Item("sender") = "IM") Then
                                ...
                                Try
                                    ...
                                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(0).Rows(L).Item("OrderID"), NormalizeData.Tables(0).Rows(L).Item("MerchantID"), NormalizeData.Tables(0).Rows(L).Item("MerchantLoginID"), NormalizeData.Tables(0).Rows(L).Item("Customer_Id"), "N", "N", PayModeMPI, "CXL_IM")
                                    If ChkLog.Checked = False Then
                                        log = ". Dumping data for MPI Reseller" & vbCrLf
                                        log = log & "Exception:" & obj.GetExcept
                                        WriteDebugInfo(log, filename)
            ...
            If (NormalizeData.Tables(1).Rows.Count > 0) Then
                ...
                For K = 0 To NormalizeData.Tables(1).Rows.Count - 1
                    ...
                    Try
                        If Blocked = True Then
                            ResponseCode_Cs = "0"
                            CsMessage = "Bank Transaction Successfull"
                        End If
                                ...
                                If (obj.CheckServiceEnable(NormalizeData.Tables(1).Rows(K).Item("MerchantID"), "118") = True Or obj.CheckServiceEnable(NormalizeData.Tables(1).Rows(K).Item("MerchantID"), "232") = True) Then
                                    ...
                                    For i = 0 To ODetail.Tables(0).Rows.Count - 1
                                        With ODetail.Tables(0).Rows(i)
                                            ...
                                            'Commented on 20/04/09 on Request of Rehan bhai
                                            'Dim product_sale_price As Double = (.Item("product_sale_price") * .Item("Qty")) - .Item("Discount")
                        ...
                        ElseIf (NormalizeData.Tables(1).Rows(K).Item("sender") = "IR" Or NormalizeData.Tables(1).Rows(K).Item("sender") = "IN" Or NormalizeData.Tables(1).Rows(K).Item("sender") = "PR") Then
                            ...
                            If ChkLog.Checked = False Then
                                ...
                                log = log & "Permission = " & System.Configuration.ConfigurationManager.AppSettings("ZeroAmountWebservice")
                                WriteDebugInfo(log, filename)
                            End If

                            Dim strCreateInv As String = "FALSE"
                            If UCase(System.Configuration.ConfigurationManager.AppSettings("ZeroAmountWebservice")) = "ON" Then
                                ...
                                If NormalizeData.Tables(1).Rows(K).Item("amount") = 0 Then
                                    ...
                                    Try
                                        ...
                                        Dim key As String = EncryptionDecryption.DecryptString128Bit(System.Configuration.ConfigurationManager.AppSettings("key"), "xxx")
                                        Dim userid As String = EncryptionDecryption.DecryptString128Bit(System.Configuration.ConfigurationManager.AppSettings("ishopswebservices_userid"), key)
                                        Dim pwd As String = EncryptionDecryption.DecryptString128Bit(System.Configuration.ConfigurationManager.AppSettings("ishopswebservices_pwd"), key)
                                        Dim domain As String = EncryptionDecryption.DecryptString128Bit(System.Configuration.ConfigurationManager.AppSettings("ishopswebservices_domain"), key)
                            ...
                            If (NormalizeData.Tables(1).Rows(K).Item("sender") = "IR") Then
                                ...
                                Try
                                    ...
                                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(1).Rows(K).Item("OrderID"), NormalizeData.Tables(1).Rows(K).Item("MerchantID"), NormalizeData.Tables(1).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(1).Rows(K).Item("Customer_Id"), strCreateInvMPI, strCreateInvMPI, "CB", "CXL_IR")
                                    If ChkLog.Checked = False Then
                                        log = ". Dumping data for MPI Reseller" & vbCrLf
                                        log = log & "Exception:" & obj.GetExcept
                                        WriteDebugInfo(log, filename)
                        ...
                        ElseIf (NormalizeData.Tables(1).Rows(K).Item("sender") = "IN") Then
                            ...
                            Try
                                ...
                                obj.MPI_ResselerAndActivationData(NormalizeData.Tables(1).Rows(K).Item("OrderID"), NormalizeData.Tables(1).Rows(K).Item("MerchantID"), NormalizeData.Tables(1).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(1).Rows(K).Item("Customer_Id"), strCreateInvMPI, strCreateInvMPI, "CB", "CXL_IN")
                                If ChkLog.Checked = False Then
                                    log = ". Dumping data for MPI Reseller" & vbCrLf
                                    log = log & "Exception:" & obj.GetExcept
                                    WriteDebugInfo(log, filename)
                        ...
                        ElseIf (NormalizeData.Tables(1).Rows(K).Item("sender") = "IM") Then
                            ...
                            Try
                                ...
                                obj.MPI_ResselerAndActivationData(NormalizeData.Tables(1).Rows(K).Item("OrderID"), NormalizeData.Tables(1).Rows(K).Item("MerchantID"), NormalizeData.Tables(1).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(1).Rows(K).Item("Customer_Id"), strCreateInvMPI, strCreateInvMPI, "CB", "CXL_IM")
                                If ChkLog.Checked = False Then
                                    log = ". Dumping data for MPI Reseller" & vbCrLf
                                    log = log & "Exception:" & obj.GetExcept
                                    WriteDebugInfo(log, filename)
                ...
                For K = 0 To NormalizeData.Tables(3).Rows.Count - 1
                    ...
                    Try
                        ...
                        If (NormalizeData.Tables(3).Rows(K).Item("sender") = "IR") Then
                            ...
                            Try
                                obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(K).Item("OrderID"), NormalizeData.Tables(3).Rows(K).Item("MerchantID"), NormalizeData.Tables(3).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(K).Item("Customer_Id"), "Y", "Y", "BP", "CXL_IR")
                                If ChkLog.Checked = False Then
                                    log = ". Dumping data for MPI Reseller" & vbCrLf
                                    log = log & "Exception:" & obj.GetExcept
                                    WriteDebugInfo(log, filename)
                        ...
                        ElseIf (NormalizeData.Tables(3).Rows(K).Item("sender") = "IM") Then
                            ...
                            Try
                                obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(K).Item("OrderID"), NormalizeData.Tables(3).Rows(K).Item("MerchantID"), NormalizeData.Tables(3).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(K).Item("Customer_Id"), "Y", "Y", "BP", "CXL_IM")
                                If ChkLog.Checked = False Then
                                    log = ". Dumping data for MPI Reseller" & vbCrLf
                                    log = log & "Exception:" & obj.GetExcept
                                    WriteDebugInfo(log, filename)
                        ...
                        ElseIf (NormalizeData.Tables(3).Rows(K).Item("sender") = "IN") Then
                            ...
                            Try
                                obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(K).Item("OrderID"), NormalizeData.Tables(3).Rows(K).Item("MerchantID"), NormalizeData.Tables(3).Rows(K).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(K).Item("Customer_Id"), "Y", "Y", "BP", "CXL_IN")
                                If ChkLog.Checked = False Then
                                    log = ". Dumping data for MPI Reseller" & vbCrLf
                                    log = log & "Exception:" & obj.GetExcept
                                    WriteDebugInfo(log, filename)
            ...
            For j = 0 To NormalizeData.Tables(2).Rows.Count - 1
                ...
                Try
                    ...
                    If (FwdToCXL = 1) Then
                        ...
                        'Dim PaymProc As String = NormalizeData.Tables(2).Rows(j).Item("PaymentProcessor")
                        ...
                        'If Sender = "PR" Then
                        '    DataS = obj.GetManualOrderDetail(NormalizeData.Tables(2).Rows(j).Item("MerchantID"), NormalizeData.Tables(2).Rows(j).Item("OrderID"))

                        'Else
                        DataS = obj.GetOrderDetail(NormalizeData.Tables(2).Rows(j).Item("MerchantID"), NormalizeData.Tables(2).Rows(j).Item("OrderID"))
                                            ...
                                            Else
                                                ...
                                                If (objPayPal.ISCertInstalled(NormalizeData.Tables(2).Rows(j).Item("PayPalID"), P_P_Password, paypalEnvironmantType) <> 1) Then
                                                    ...
                                                    If (IsInstalled = "1") Then
                                                        ...
                                                        obj.GETRESPONSEFROMPAYPAL(NormalizeData.Tables(2).Rows(j).Item("PayPalID"), P_P_Password, paypalEnvironmantType, NormalizeData.Tables(2).Rows(j).Item("CreditCardNo"), NormalizeData.Tables(2).Rows(j).Item("CardType"), NormalizeData.Tables(2).Rows(j).Item("SecurityCode"), NormalizeData.Tables(2).Rows(j).Item("CardExpiry"), NormalizeData.Tables(2).Rows(j).Item("Amount"), NormalizeData.Tables(2).Rows(j).Item("CurrencyCode"), NormalizeData.Tables(2).Rows(j).Item("MerchantID"), NormalizeData.Tables(2).Rows(j).Item("OrderID"), NormalizeData.Tables(2).Rows(j).Item("TrackID"), CxlR_code, Cxl_Msg, NormalizeData.Tables(2).Rows(j).Item("Customer_ID"))
                                                    Else
                                                        CxlR_code = "-10"
                                                        Cxl_Msg = "Credit Card is Declined Because Certificate is not Installed"
                                                    End If
                                                ...
                                                Else
                                                    ...
                                                    If ChkLog.Checked = False Then
                                                        ...
                                                        log = log & "@First Name= " & NormalizeData.Tables(2).Rows(j).Item("CardName") & vbCrLf
                                                        log = log & "@OrderID = " & NormalizeData.Tables(2).Rows(j).Item("OrderID") & vbCrLf
                                                        log = log & "@TrackID= " & NormalizeData.Tables(2).Rows(j).Item("TrackID")
                                                        WriteDebugInfo(log, filename)
                                                    End If
                        ...
                        Dim EmptyEndDate As Boolean = False
                        If NormalizeData.Tables(2).Rows(j).Item("EndDate") = "" Then
                            NormalizeData.Tables(2).Rows(j).Item("EndDate") = "1/1/2000"
                            EmptyEndDate = True
                        End If
                        ...
                        If ChkLog.Checked = False Then
                            ...
                            WriteDebugInfo(log, filename)
                        End If
                        If EmptyEndDate = True Then
                            EmptyEndDate = False
                            If ChkLog.Checked = False Then
                                log = ". Update Credit Card Details by Calling UpdateCreditCardDetail. EmptyEndDate =  True "
                                WriteDebugInfo(log, filename)
                                obj.GetExcept = ""
                            End If

                            obj.UpdateCreditCardDetailInInvoice(NormalizeData.Tables(2).Rows(j).Item("MerchantID"), NormalizeData.Tables(2).Rows(j).Item("Customer_ID"), NormalizeData.Tables(2).Rows(j).Item("CreditCardNo"), NormalizeData.Tables(2).Rows(j).Item("CardName"), NormalizeData.Tables(2).Rows(j).Item("CardType"), NormalizeData.Tables(2).Rows(j).Item("CardExpiry"), NormalizeData.Tables(2).Rows(j).Item("CardAddress"), NormalizeData.Tables(2).Rows(j).Item("OrderID"), NormalizeData.Tables(2).Rows(j).Item("SecurityCode"), NormalizeData.Tables(2).Rows(j).Item("StartDate"), NormalizeData.Tables(2).Rows(j).Item("EndDate"), NormalizeData.Tables(2).Rows(j).Item("IssueNo"), Key)

                            If ChkLog.Checked = False Then
                                log = ". Update Credit Card Details by Calling UpdateCreditCardDetail End. "
                                log = log & ". Except:" & obj.GetExcept
                                WriteDebugInfo(log, filename)
                                obj.GetExcept = ""
                            End If
            ...
            If (OrderID > 0 And (Sender = "AC" Or Sender = "IN" Or Sender = "FH" Or Sender = "IR" Or Sender = "IB" Or Sender = "IM" Or Sender = "PR")) Then 'it is orderbooked
                ...
                If MPI_SessionID Is Nothing Then
                    If retryCount >= CInt(counter) - 1 Then 'Added by Saad for counting the number of calls fro particular order (03/01/08)
                        ...
                        log = log & "---- Order has been declined after " & retryCount & " number of calls" & vbCrLf
                        log = log & ". Except:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
                        obj.GetExcept = ""
                        SendEmail("Order ID: " & OrderID & " and Merchant ID" & MerchantId & " has been declined after " & retryCount & " number of calls")
                    End If
                Else
                    'If retryCount > 1 Then
                    If retryCount >= CInt(counter) - 1 Then 'since it declines order on 2nd try, so, as per ri instructions it is modified on 03122014
                ...
                If ChkLog.Checked = False Then
                    ...
                    log = log & ". @OrderID= " & OrderID & vbCrLf
                    Try
                        log = log & ". OrderDataCount= " & OrderData.Tables(0).Rows.Count & vbCrLf
                    Catch ex As Exception
                    End Try
                    log = log & ". Except: " & obj.GetExcept
                    WriteDebugInfo(log, filename)
                ...
                ElseIf dtledgerCheck = True Then
                    Check = False
                Else
                    Check = CheckValidityOfAmount(Dbname, Amount, rid, OrderID, ProcessType, Authorized, MerchantId)
                End If
                '////////////////////////////////////
...
#Region "........Checking Validity Of Amount of a Particular Customer .........."
        ...
        Private Function CheckValidityOfAmount(ByVal Dbname As String, ByVal Amt As Double, ByVal Rid As Long, ByVal OrderID As String, ByVal ProcessType As String, ByVal auth As Integer, ByVal Mid As String) As Boolean
            ...
            Try
                ...
                Dim DTab As DataTable = obj.GetValidTranDrCr(Dbname, Rid, Mid)
            ...
            If (EnDs.Tables(0).Rows.Count > 0) Then
                ...
                For p = 0 To EnDs.Tables(0).Rows.Count - 1
                    ...
                    If (Cardno <> "" And i = -1) Then
                        ...
                        E = InfiniLogic.AccountsCentre.common.ConfigReader.GetItem(InfiniLogic.AccountsCentre.common.ConfigVariables.E)
                        N = InfiniLogic.AccountsCentre.common.ConfigReader.GetItem(InfiniLogic.AccountsCentre.common.ConfigVariables.N)
                        ECard = ObjCrypt.Encrypt(Cardno, E, N)
                        ECard = Replace(ECard, "+", "p")
                        If (IssueNum <> "") Then

                            EIssueNum = ObjCrypt.Encrypt(IssueNum, E, N)
                            EIssueNum = Replace(EIssueNum, "+", "p")
                        End If
                        If (securityCode <> "") Then

                            ESecurityCode = ObjCrypt.Encrypt(securityCode, E, N)
                            ESecurityCode = Replace(ESecurityCode, "+", "p")
...
#Region ".............. Function For Payment is Full OR Partial................."
        ...
        Private Function CheckPaymentIsFullOrPartial(ByVal mid As String, ByVal orderid As String, ByVal PayAmount As Double, ByRef PaidAmount As Double) As Boolean
            Try
                ...
                'If Sender = "PR" Then
                '    ODetail = obj.GetManualOrderDetail(mid, orderid)
                'Else
                ODetail = obj.GetOrderDetail(mid, orderid)
                'End If
                ...
                Next
                ' SUM SHIPMENT AMOUNT 
                '//// add format to round values in two decimal
                'Dim SumCartOrderAmount As Double = (SumProductsAmount + SumCarriageCharges) - DiscountAmount
                Dim SumCartOrderAmount As Double = Format((SumProductsAmount + SumCarriageCharges) - DiscountAmount, "0.00")
        ...
        Private Sub TimerPopulate_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TimerPopulate.Tick
            ...
            If (WaitToComplete = False) Then
                ...
                Try
                    ...
                    If ChkLog.Checked = False Then
                        timestring = Date.Now
                        folder = "d:\Log-SingleDB\" & Date.Today
                ...
                ' No Longer Required as "MainServiceMethod" Method is obsolete and deleted
                'Try
                '    If UCase(System.Configuration.ConfigurationManager.AppSettings("PostInfinishop")) = "ON" Then
...
#Region "........Folder Creation methods and log files maintain functions......."
        Private Sub CreateFolder(ByVal vfolder As String)
            Try
                Dim fs, f As Object
        ...
        Sub WriteStrToFile(ByVal MyKeyString As String, ByVal strFileName As String)
            ...
            Try
                ...
                objReader.Close()
            Catch ex As Exception
                LstException.Items.Add(DateTime.Now & " " & ex.Message & "Write to File Function")
            End Try
        End Sub

        Private Sub WriteDebugInfo(ByVal sText As String, ByVal LogUniquePath As String)
            ' Dim TraceOption As String = System.Configuration.ConfigurationManager.AppSettings("IOTraceEnable").ToString

            '     If TraceOption.Equals("1") Then
            ...
            log = ""
            '   ElseIf TraceOption.Equals("2") Then
            '  HttpContext.Current.Trace.Warn(Now & " -- " & sText)
            '  End If
        End Sub
#End Region
...
#Region "....................... Sending Error Email ..........................."
        ...
        Private Sub SendEmail(ByVal msg As String, Optional ByVal Subject As String = "Exception From Cxl Occured (LOCAL SERVER-SINGLE DB) @ Time :")

            Dim email_to As String '= System.Configuration.ConfigurationManager.AppSettings("email_to")
            Dim email_Subject As String = "Exception occured (LIVE SERVER-MULTIPLE DB)@ Time : " & Now
            Try
                email_to = System.Configuration.ConfigurationManager.AppSettings("email_to")

                If UCase(System.Configuration.ConfigurationManager.AppSettings("emailFrom")) = "LOCAL" Then
                    email_Subject = "Exception occured (LOCAL SERVER-MULTIPLE DB)@ Time : " & Now
                End If

                obj.InsertEmailData("errors@cxl.com", email_to, email_Subject, msg, "", "")

            Catch exx As Exception

                LstException.Items.Add((DateTime.Now & " " & "The following exception occurred at SendEmail : " + exx.ToString()))

                Dim objmail As New System.Net.Mail.MailMessage
                Dim smtp As New System.Net.Mail.SmtpClient
                email_to = System.Configuration.ConfigurationManager.AppSettings("email_to")

                objmail.From = New System.Net.Mail.MailAddress("errors@cxl.com")

                Dim Addresses() As String = Split(email_to, ",")
                Dim a As Integer
                For a = Addresses.GetLowerBound(0) To Addresses.GetUpperBound(0)
                    objmail.To.Add(New System.Net.Mail.MailAddress(Addresses(a)))
                Next

                'objmail.Cc = "saad@infinilogic.com"

                If UCase(System.Configuration.ConfigurationManager.AppSettings("emailFrom")) = "LOCAL" Then
                    objmail.To.Add(New System.Net.Mail.MailAddress("database.errors@infinilogic.com"))
                    objmail.Subject = "Exception occured (LOCAL SERVER-SINGLE DB)@ Time : " & Now
                    smtp.Host = "192.168.1.66"  'Local Server
                Else
                    objmail.Subject = "Exception occured (LIVE SERVER-SINGLE DB)@ Time : " & Now
                    smtp.Host = "10.0.0.42"  'Live Server"
                End If
                objmail.Body = msg
        ...
        '    ''objmail.To = "anjum@infinilogic.com"       '"sadiawaleem@infinilogic.com"
        '    ''objmail.Subject = "Exception occured @ Time : " & Now
        '    ''objmail.Body = msg
        '    ''SmtpMail.SmtpServer = "192.168.1.66" 'Live Server= "10.0.0.42" ' LOCAL server="213.86.130.41"
        '    ''"10.0.0.42" ''on local server "213.86.130.41" '//Give ur mail servers IP address here
        '    '' Try
        ...
        '    objmail.From = "errors@cxl.com"
        '    objmail.To = "sadiawaleem@infinilogic.com" '"sadiawaleem@infinilogic.com"
        '    objmail.Subject = Subject & Now
        '    objmail.Body = msg
        '    SmtpMail.SmtpServer = "192.168.1.66" '"213.86.130.41" 'Live Server= "10.0.0.42" ' LOCAL server="213.86.130.41"
        '    '"10.0.0.42" ''on local server "213.86.130.41" '//Give ur mail servers IP address here
        '    Try
        '        SmtpMail.Send(objmail)
        '    Catch ex As Exception
        '        LstException.Items.Add(("The following exception occurred: " + ex.ToString()))

        '        While Not (ex.InnerException Is Nothing)
        '            LstException.Items.Add(("The following InnerException reported: " + ex.InnerException.ToString()))
        '            ex = ex.InnerException
        '        End While
        '    End Try
...
#Region "................ List Exception Selected Text Methods.................."
        ...
        Protected Sub ContextHandler(ByVal sender As System.Object, ByVal e As System.EventArgs)
            ...
            Const LVW_COPY As String = "&Copy To Clip Board"
            Const LIST_BOX_NAME As String = "LstException"
...
#End Region
        ...
        'Shared Function WriteToEventLog(ByVal Entry As String, Optional ByVal AppName As String = "MicroScope", Optional ByVal EventType As EventLogEntryType = EventLogEntryType.Information, Optional ByVal LogName As String = "Application") As Boolean
...
#Region "................ Updation on the basis Of CXL Code....................."
        ...
        Private Sub SuccessfulCXL(ByVal J As Integer, ByVal CXLR_Code As String, ByVal CXL_Msg As String)
            ...
            ElseIf (CorporateOrder = True) Then
                ...
                '/////// Start of changes as per ri instruciton on 28102014
                ...
                '''''''''''''Check is Over dues mts charges order Start Changes by greesh 07-05-2012 
                If ChkLog.Checked = False Then
                    log = log & " Start GetMTS_OverDuesChargesRows:"
                    WriteDebugInfo(log, filename)
                End If

                Dim dstMTS_OverDues As DataSet
                If NormalizeData.Tables(2).Rows(J).Item("MerchantID") = System.Configuration.ConfigurationManager.AppSettings("MTSAdvanced") Then dstMTS_OverDues = obj.GetMTS_OverDuesChargesRows(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"))
                If Not dstMTS_OverDues Is Nothing AndAlso dstMTS_OverDues.Tables.Count > 0 AndAlso dstMTS_OverDues.Tables(0).Rows.Count > 0 Then
                    Dim X As Integer = 0
                    'For X = 0 To DataS.Tables(0).Rows.Count - 1
                    obj.OrderAddDetailForMTS(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"), "@ORD" + NormalizeData.Tables(2).Rows(J).Item("OrderID"), DataS.Tables(0).Rows(X).Item("Product_ID"), DataS.Tables(0).Rows(X).Item("prod_code"), DataS.Tables(0).Rows(X).Item("Qty"), DataS.Tables(0).Rows(X).Item("product_sale_price"), DataS.Tables(0).Rows(X).Item("product_tax_code"), DataS.Tables(0).Rows(X).Item("tax_code_rate"), DataS.Tables(0).Rows(X).Item("unitprice"), "S", dstMTS_OverDues.Tables(0).Rows(X)("MTSInvoiceRef"))
                    If ChkLog.Checked = False Then
                        log = ". Adding OrderAddDetailForMTS Order Detail . "
                        log = log & ". Except:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
                    End If
                    'Next
                    If ChkLog.Checked = False Then
                        log = log & " End GetMTS_OverDuesChargesRows:"
                        WriteDebugInfo(log, filename)
                    End If
                ...
                Else
                    ...
                    Dim X As Integer = 0
                    'For X = 0 To DataS.Tables(0).Rows.Count - 1
                    obj.SrvOrderAddDetail(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"), "@ORD" + NormalizeData.Tables(2).Rows(J).Item("OrderID"), DataS.Tables(0).Rows(X).Item("Product_ID"), DataS.Tables(0).Rows(X).Item("prod_code"), DataS.Tables(0).Rows(X).Item("Qty"), DataS.Tables(0).Rows(X).Item("product_sale_price"), DataS.Tables(0).Rows(X).Item("product_tax_code"), DataS.Tables(0).Rows(X).Item("tax_code_rate"), DataS.Tables(0).Rows(X).Item("unitprice"), "S")
                    If ChkLog.Checked = False Then
                        log = ". Adding SrvOrderAddDetail Order Detail . "
                        log = log & ". Except:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
                    End If
                    'Next
                End If
                '''''''''''''ENd of Check is Over dues mts charges order Changes by greesh 07-05-2012 


                ' ............... end Insert order details in inv_det table..........."
                '/////// End   of changes as per ri instruciton on 28102014
                ...
                Try
                    ...
                    Catch ex As Exception
                        ...
                        WriteDebugInfo(log, filename)
                        SendEmail("Exception:. Coupon Webservice:" & ex.Message & " , Merchant ID = " & NormalizeData.Tables(2).Rows(J).Item("MerchantID") & " , Order ID = " & NormalizeData.Tables(2).Rows(J).Item("OrderID"))
                    End Try
                    Try

                        obj.MPI_ResselerAndActivationData(NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(J).Item("Customer_Id"), "Y", "Y", "CC", "CXL_IR")
                        If ChkLog.Checked = False Then
                            log = ". Dumping data for MPI Reseller" & vbCrLf
                            log = log & "Exception:" & obj.GetExcept
                            WriteDebugInfo(log, filename)
            ...
            Else
                ...
                If (UCase(NormalizeData.Tables(2).Rows(J).Item("ProcessType")) = "CC") Then
                    ...
                    If ChkLog.Checked = False Then
                        log = log & " start  UpdateStatusMTS_OverDuesCharges_reference:"
                        WriteDebugInfo(log, filename)
                    End If
                '''''''''''''''''''''''''Start Greesh 27-08-2012

                obj.UpdateStatusMTS_LenderORder(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"), Inv_no, NormalizeData.Tables(2).Rows(J).Item("TrackID"))

                '''''''''''''''''''''''''End Greesh 27-08-2012 MTS LEnder Update Order------------------


                    ''''''''''''''''Start Greesh 07-05-2012 For MTS ORder Overdues Charges Paid ''''''''''''
                If NormalizeData.Tables(2).Rows(J).Item("MerchantID") = System.Configuration.ConfigurationManager.AppSettings("MTSAdvanced") Then
                    obj.UpdateStatusMTS_OverDuesCharges_reference(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"))
                    ''''''''''''''''End Greesh 07-05-2012 For MTS ORder Overdues Charges Paid ''''''''''''

                    If ChkLog.Checked = False Then
                        log = log & " End  UpdateStatusMTS_OverDuesCharges_reference:"
                        WriteDebugInfo(log, filename)
                    End If

                    ''''''''''''''''Start Greesh 07-05-2012 For MTS ORder Overdues Charges Paid ''''''''''''
                    If ChkLog.Checked = False Then
                        log = log & " start  MTS_CXL_Get_Approved_PurchaseORder:"
                        WriteDebugInfo(log, filename)
                    End If
                    obj.MTS_CXL_Get_Approved_PurchaseORder(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("Amount"), NormalizeData.Tables(2).Rows(J).Item("TrackID"))
                    ''''''''''''''''End Greesh 07-05-2012 For MTS ORder Overdues Charges Paid ''''''''''''
                ...
                If ChkLog.Checked = False Then
                    log = log & " End  MTS_CXL_Get_Approved_PurchaseORder:"
                    WriteDebugInfo(log, filename)
                End If
            ...
            ElseIf (NormalizeData.Tables(2).Rows(J).Item("sender") = "IR" And CorporateOrder = False) Then
                ...
                Try
                    ...
                    Dim addSupplierResult As com.infinishops.addSupplier.ErrorCodesInfo = addSupplier.CallAddSupplier(NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("Customer_id"), NormalizeData.Tables(2).Rows(J).Item("OrderID"))
            ...
            ElseIf (NormalizeData.Tables(2).Rows(J).Item("sender") = "IN" And CorporateOrder = False) Then
                ...
                Try
                    ...
                    Try
                        obj.MPI_ResselerAndActivationData(NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(J).Item("Customer_Id"), "Y", "Y", "CC", "CXL_IN")
                        If ChkLog.Checked = False Then
                            log = ". Dumping data for MPI Reseller" & vbCrLf
                            log = log & "Exception:" & obj.GetExcept
                            WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(2).Rows(J).Item("sender") = "IM" And CorporateOrder = False) Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(J).Item("Customer_Id"), "Y", "Y", "CC", "CXL_IM")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
        ...
        Private Sub SuccessfulPayOnCredit(ByVal J As Integer, ByVal CXLR_Code As String, ByVal CXL_Msg As String)
            ...
            If Not ds Is Nothing Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    log = log & ". @OrderID= " & NormalizeData.Tables(4).Rows(J).Item("OrderID") & vbCrLf
                    Try
                        Dim dsReferal As DataSet = obj.REFERRAL_ATTRIBUTE(NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("OrderID"), "DC2", "1")
                        If dsReferal.Tables(0).Rows.Count > 0 Then
                            Dim dsPay As DataSet = obj.REFERRAL_PAYCOMMISSION_For_DC2(NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("Customer_Id"), NormalizeData.Tables(4).Rows(J).Item("OrderID"))
                            log = log & "PayCommission Message ID For DC2: " & dsPay.Tables(0).Rows(0).Item("MessageID") & vbCrLf
                            log = log & "PayCommission Message Text For DC2: " & dsPay.Tables(0).Rows(0).Item("MessageText")
                        Else
            ...
            ElseIf (NormalizeData.Tables(4).Rows(J).Item("sender") = "IR") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(4).Rows(J).Item("OrderID"), NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(4).Rows(J).Item("Customer_Id"), "Y", "Y", "CR", "CXL_IR")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
                ...
                Try
                    ...
                    Dim addSupplier As New com.infinishops.addSupplier.AddSupplier
                    Dim addSupplierResult As com.infinishops.addSupplier.ErrorCodesInfo = addSupplier.CallAddSupplier(NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("Customer_id"), NormalizeData.Tables(4).Rows(J).Item("OrderID"))
            ...
            ElseIf (NormalizeData.Tables(4).Rows(J).Item("sender") = "IN") Then
                ...
                Try
                    ...
                    Try
                        obj.MPI_ResselerAndActivationData(NormalizeData.Tables(4).Rows(J).Item("OrderID"), NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(4).Rows(J).Item("Customer_Id"), "Y", "Y", "CR", "CXL_IN")
                        If ChkLog.Checked = False Then
                            log = ". Dumping data for MPI Reseller" & vbCrLf
                            log = log & "Exception:" & obj.GetExcept
                            WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(4).Rows(J).Item("sender") = "IM") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(4).Rows(J).Item("OrderID"), NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(4).Rows(J).Item("Customer_Id"), "Y", "Y", "CR", "CXL_IM")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
            ...
            'Reset ccStatus Value Y to N
            Try
                obj.UpDatePayAmount(NormalizeData.Tables(4).Rows(J).Item("MerchantID"), NormalizeData.Tables(4).Rows(J).Item("OrderID"), 0, "N")
            Catch ex As Exception
                log = "Reset ccStatus Value Y to N" & vbCrLf
                log = log & " Exception : " & ex.Message
                WriteDebugInfo(log, filename)
            End Try
        ...
        Private Sub SuccessfulCXLFOR_BP(ByVal J As Integer, ByVal CXLR_Code As String, ByVal CXL_Msg As String)
            ...
            Dim Inv_no As String = Invoice_no
        ...
        Private Sub SuccessfulCXLFOR_PA(ByVal J As Integer, ByVal CXLR_Code As String, ByVal CXL_Msg As String)
            ...
            ElseIf (NormalizeData.Tables(3).Rows(J).Item("sender") = "IR") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(J).Item("OrderID"), NormalizeData.Tables(3).Rows(J).Item("MerchantID"), NormalizeData.Tables(3).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(J).Item("Customer_Id"), "Y", "Y", "PA", "CXL_IR")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(3).Rows(J).Item("sender") = "IM") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(J).Item("OrderID"), NormalizeData.Tables(3).Rows(J).Item("MerchantID"), NormalizeData.Tables(3).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(J).Item("Customer_Id"), "Y", "Y", "PA", "CXL_IM")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(2).Rows(J).Item("sender") = "IR") Then
                ...
                Try
                    ...
                    Else
                        PayModeMPI = "CB"
                    End If
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(J).Item("Customer_Id"), "N", "N", PayModeMPI, "CXL_IR")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(2).Rows(J).Item("sender") = "IM") Then
                ...
                Try
                    ...
                    Else
                        PayModeMPI = "CB"
                    End If
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(2).Rows(J).Item("OrderID"), NormalizeData.Tables(2).Rows(J).Item("MerchantID"), NormalizeData.Tables(2).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(J).Item("Customer_Id"), "N", "N", PayModeMPI, "CXL_IM")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
        ...
        Private Sub DeclineForPA(ByVal J As Integer, ByVal CxlR_Code As String, ByVal Cxl_Msg As String)
            ...
            ElseIf (NormalizeData.Tables(3).Rows(J).Item("sender") = "IR") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(J).Item("OrderID"), NormalizeData.Tables(3).Rows(J).Item("MerchantID"), NormalizeData.Tables(3).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(J).Item("Customer_Id"), "N", "N", "PA", "CXL_IR")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
            ...
            ElseIf (NormalizeData.Tables(3).Rows(J).Item("sender") = "IM") Then
                ...
                Try
                    obj.MPI_ResselerAndActivationData(NormalizeData.Tables(3).Rows(J).Item("OrderID"), NormalizeData.Tables(3).Rows(J).Item("MerchantID"), NormalizeData.Tables(3).Rows(J).Item("MerchantLoginID"), NormalizeData.Tables(3).Rows(J).Item("Customer_Id"), "N", "N", "PA", "CXL_IM")
                    If ChkLog.Checked = False Then
                        log = ". Dumping data for MPI Reseller" & vbCrLf
                        log = log & "Exception:" & obj.GetExcept
                        WriteDebugInfo(log, filename)
