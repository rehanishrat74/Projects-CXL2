...
#Region " Windows Form Designer generated code "
        ...
        <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
            ...
            Me.ClearEX.Size = New System.Drawing.Size(75, 23)
            ...
            Me.Text = "CXL ROBOT 2.0 (MultiDB) 05.11.10"
...
#Region "..................... Public Variable............................."
        ...
        <CLSCompliant(False)> Public ObjInifiniMarketPlace As InfiniMarket._Default
...
#Region " .... MYSQL fetching Data and inserting into Sql SERVER and updating MYSQL Status after getting Cxl Code...."
        ...
        Private Sub MySqlData()
            ...
            Try
                ...
                LstException.Items.Add("total Rows" & mysqlds.Tables(0).Rows.Count)
                ...
                If (mysqlds.Tables(0).Rows.Count > 0) Then
                    ...
                    For i = 0 To mysqlds.Tables(0).Rows.Count - 1
                        ...
                        Try
                            If (cardno = "" And cardname = "" And cardaddress = "" And securitycode = "") Or _
                            obj.Check_MerchantTNSPayEnabled(2) = False Then
                        ...
                        Catch ex As Exception
                            'MsgBox(ex.Message & "exstack" & ex.StackTrace)
        ...
        Private Sub UpdateMySqlStatus(ByVal C_id As String, ByVal O_id As String, ByVal Cxl_code As String, ByVal Cxl_msg As String, ByVal Cs_msg As String, ByVal T_id As String, ByVal StatusFromCs As String, ByVal Responsecode_cs As String, Optional ByVal Inv_no As String = "NULL")
            Try
                ...
                'commented on 04092014
                'obj.UpdateMySqlData("UPDATE calls_atcs SET Cxlcode='" & Cxl_code & "',cxlmessage= '" & Cxl_msg & "',csmessage='" & Cs_msg & "',trackid_atcs= " & T_id & ",statusfromcs='" & StatusFromCs & "' ,Responsecode_cs=" & Responsecode_cs & " ,Processed_time='" & a & "',Invoiceno='" & Inv_no & "' WHERE ORDERBOOKED=" & O_id & " And loginid='" & C_id & "' And (statusfromCs='I' or statusfromcs='P' or statusfromcs='3D' or statusfromcs='S' or statusfromcs='5')")

                '/// it is added on 10092014
                If ChkLog.Checked = False Then
                    log = ". UPDATE calls_atcs SET Cxlcode='" & Cxl_code & "',cxlmessage= '" & Cxl_msg & "',csmessage='" & Cs_msg & "',trackid_atcs= " & T_id & ",statusfromcs='" & StatusFromCs & "' ,Responsecode_cs=" & Responsecode_cs & " ,Processed_time='" & a & "',Invoiceno='" & Inv_no & "' WHERE ORDERBOOKED=" & O_id & " And loginid='" & C_id & "' And (statusfromCs='I' or statusfromcs='P' or statusfromcs='3D' or statusfromcs='S' or statusfromcs='5' or statusfromcs='D')"
                    WriteDebugInfo(log, filename)
                End If

                obj.UpdateMySqlData("UPDATE calls_atcs SET Cxlcode='" & Cxl_code & "',cxlmessage= '" & Cxl_msg & "',csmessage='" & Cs_msg & "',trackid_atcs= " & T_id & ",statusfromcs='" & StatusFromCs & "' ,Responsecode_cs=" & Responsecode_cs & " ,Processed_time='" & a & "',Invoiceno='" & Inv_no & "' WHERE ORDERBOOKED=" & O_id & " And loginid='" & C_id & "' And (statusfromCs='I' or statusfromcs='P' or statusfromcs='3D' or statusfromcs='S' or statusfromcs='5' or statusfromcs='D')")
...
#End Region
        ...
        Private Sub BindGrid()
            ...
            Catch ex As Exception
                'MsgBox(ex.Message & "ex.stacktrace" & ex.StackTrace)
                    ...
                    For i = 0 To Ds.Tables(0).Rows.Count - 1
                        ...
                        If (CSAvailable <> 0) Then
                            ...
                            If (Check = True) Then
                                ...
                                ElseIf (UCase(ProcessType) = "CB") And OrderDataExists = True Then '///if orderdata is not filled 
                    ...
                    Try
                        ...
                        ElseIf (NormalizeData.Tables(0).Rows(L).Item("sender") = "AC") Then
                            ...
                            If ChkLog.Checked = False Then
                                ...
                                log = log & ". @MerchantID= " & NormalizeData.Tables(0).Rows(L).Item("MerchantID") & vbCrLf
                                log = log & ". @OrderID= " & NormalizeData.Tables(0).Rows(L).Item("OrderID") & vbCrLf
                                log = log & ". @CXLMessage= " & IIf(Cust_ID = 0 And OrderDataExists = False, CustomerIdMissing, "NULL") & vbCrLf
                                log = log & ". @CXLCode= " & 500 & vbCrLf
                                log = log & ". @TrackID= " & NormalizeData.Tables(0).Rows(L).Item("TrackID") & vbCrLf
                                log = log & ". @Sender= " & NormalizeData.Tables(0).Rows(L).Item("sender") & vbCrLf
                                log = log & ". @CCStatus= " & "D" & vbCrLf
            ...
            If (NormalizeData.Tables(1).Rows.Count > 0) Then
                ...
                For K = 0 To NormalizeData.Tables(1).Rows.Count - 1
                    Try
                        ...
                        If rid = 0 Then
                            ResponseCode_Cs = "0"
                            CsMessage = CsMessage & ": Bank Transaction is Tested Successfully Invoice Will Create"
                        Else
                            ResponseCode_Cs = "0"
                        ...
                        ElseIf (NormalizeData.Tables(1).Rows(K).Item("sender") = "IR" Or NormalizeData.Tables(1).Rows(K).Item("sender") = "IN" Or NormalizeData.Tables(1).Rows(K).Item("sender") = "PR") Then
                            ...
                            If UCase(System.Configuration.ConfigurationManager.AppSettings("ZeroAmountWebservice")) = "ON" Then
                                ...
                                Dim iShopWS As com.webservices.ishops.OrderProcessing = Nothing
...
#Region ".................... Credit Card Transaction .........................."
        ...
        Public Sub CreditCardStatusUpdation()
            ...
            Dim TrackidMessage As String = ""
            CxlR_code = ""
            Cxl_Msg = ""
            'CsMessage = ""
            'ResponseCode_Cs = ""
            If rid = 0 Then
                TrackidMessage = "TrackId is not generated | "
            End If
            ...
            For j = 0 To NormalizeData.Tables(2).Rows.Count - 1
                Try
                    ...
                    If (FwdToCXL = 1) Then
                            ...
                            Case "CXL"
                                ...
                                If (NormalizeData.Tables(2).Rows(j).Item("NoCXL") = "0") Then
                                    ...
                                    If (NormalizeData.Tables(2).Rows(j).Item("Createinvoice") = "Y" Or NormalizeData.Tables(2).Rows(j).Item("Createinvoice") = "y") Then
                                        ...
                                        If tempNo = 0 Then
                                            ...
                                            Else
                                                If (CheckValidParameter(NormalizeData.Tables(2).Rows(j).Item("CreditCardNo"), NormalizeData.Tables(2).Rows(j).Item("IssueNo"), NormalizeData.Tables(2).Rows(j).Item("TransactionAmount"), NormalizeData.Tables(2).Rows(j).Item("GCode"), NormalizeData.Tables(2).Rows(j).Item("MerchantID"), NormalizeData.Tables(2).Rows(j).Item("Customer_ID"), NormalizeData.Tables(2).Rows(j).Item("OrderID"), NormalizeData.Tables(2).Rows(j).Item("TransactionType"), NormalizeData.Tables(2).Rows(j).Item("StartDate"), NormalizeData.Tables(2).Rows(j).Item("CardExpiry"), NormalizeData.Tables(2).Rows(j).Item("CardType"), NormalizeData.Tables(2).Rows(j).Item("Mode"), CxlR_code, Cxl_Msg, NormalizeData.Tables(2).Rows(j).Item("AgentName"), NormalizeData.Tables(2).Rows(j).Item("AgentAcquirer"), NormalizeData.Tables(2).Rows(j).Item("TrackID"), NormalizeData.Tables(2).Rows(j).Item("MerchantLoginID"), NormalizeData.Tables(2).Rows(j).Item("CLoginID"), NormalizeData.Tables(2).Rows(j).Item("Sender"), NormalizeData.Tables(2).Rows(j).Item("SecurityCode")) = True) Then
                                                    WriteDebugInfo("calling GETRESPONSEFROMCXL", filename)
                                                    ...
                                                    WriteDebugInfo("end GETRESPONSEFROMCXL", filename)
                                                End If
                                            End If
                                        ...
                                        End If
                                        WriteDebugInfo("calling cxlr code1", filename)
                                        CxlR_code = Trim(CxlR_code)
                                        WriteDebugInfo("calling cxlr code", filename)
                                    ...
                                    Else
                                        ...
                                        If (NormalizeData.Tables(2).Rows(j).Item("ccstatus") = "Y") Then ' that means full payment
                                            If ChkLog.Checked = False Then
                                                ...
                                                obj.GetExcept = ""
                                            End If
                                            WriteDebugInfo("calling update invoice status", filename)
                                        ...
                                        Else
                                            If ChkLog.Checked = False Then
                                                ...
                                                obj.GetExcept = ""
                                            End If
                                            WriteDebugInfo("calling update invoice status 2", filename)
                                        ...
                                        If ChkLog.Checked = False Then
                                            ...
                                            WriteDebugInfo(log, filename)
                                        End If
                                        CsMessage = TrackidMessage & "Invoice will not create"
                                        ResponseCode_Cs = "-7"
                                        CxlR_code = "-200"
                                        Cxl_Msg = "Invoice will not create"
                                        WriteDebugInfo("calling invoce will not crate", filename)
                                ...
                                If (NormalizeData.Tables(2).Rows(j).Item("NoCXL") = "0") Then
                                    ...
                                    If (NormalizeData.Tables(2).Rows(j).Item("ByForce") = "N") Then
                                        ...
                                        If (NormalizeData.Tables(2).Rows(j).Item("Createinvoice") = "Y" Or NormalizeData.Tables(2).Rows(j).Item("Createinvoice") = "y") Then
                                            ...
                                            End If
                                        Else
                                            WriteDebugInfo("calling rehan ccstatus=y", filename)
                                            If (NormalizeData.Tables(2).Rows(j).Item("ccstatus") = "Y") Then ' that means full payment
                                                If ChkLog.Checked = False Then
                                                    log = ". Update Invoice Status of CreditCard " & vbCrLf
                                                    log = log & ". by SP: CAM_UpdateInvoiceStatus . " & vbCrLf
                        ...
                        CxlR_code = Trim(CxlR_code)
                        If rid = 0 Then
                            Cxl_Msg = "TrackId is not generated | " & Cxl_Msg
                        End If
                        ...
                        WriteDebugInfo("calling get mckey", filename)
                        ...
                        ElseIf Account = "PP" Then
                            ...
                            splitter = CXL_Digit.Split(" ")
                        End If
                        WriteDebugInfo("calling splitter", filename)
                        ...
                        ElseIf (CxlR_code = "") Then
                            WriteDebugInfo("calling update response new", filename)
                            ...
                            If (NormalizeData.Tables(2).Rows(j).Item("sender") = "FH") Then
                                If ChkLog.Checked = False Then
                                    ...
                                    WriteDebugInfo(log, filename)
                                End If
                                WriteDebugInfo("calling update mysql", filename)
                ...
                Catch ex As Exception
                    ...
                    ResponseCode_Cs = "-6"    ' Credit card not posted to cxl bcoz of any failure
                    CsMessage = TrackidMessage & "Transaction is Not Forwarded to CXL B/C" & ex.Message
        ...
        Private Sub FurtherProcessing()
            ...
            If (OrderID > 0 And (Sender = "AC" Or Sender = "IN" Or Sender = "FH" Or Sender = "IR" Or Sender = "IB" Or Sender = "IM" Or Sender = "PR")) Then 'it is orderbooked
                ...
                If (rid = 0) Then
                    ...
                    '//BEGIN Changes made to set for TrackId 0 as per RI Instructions on 15072015
                    If rid = 0 Then
                        CsMessage = "TrackId is Not Generated"
                        If ChkLog.Checked = False Then
                            log = ". Trackid= " & rid & vbCrLf
                            log = log & ". CSMessage " & CsMessage & vbCrLf
                            WriteDebugInfo(log, filename)
                            obj.GetExcept = ""
                        End If
                        SendEmail("Sender = " & Sender & " OrderID= " & OrderID & " MerchantId = " & MerchantId & " TrackId = " & rid & " Exception= " & CsMessage)
                    End If
                    '//END Changes made to set for TrackId 0 as per RI Instructions on 15072015
...
#Region "......Folder Creation methods and log files maintain functions........."
        ...
        Private Sub CreateFolder(ByVal vfolder As Object)
            Try
...
#Region "..................... Sending Error Email ............................."
        ...
        Private Sub SendEmail(ByVal msg As String, Optional ByVal Subject As String = "Exception From Cxl Occured (LIVE SERVER) @ Time :")
            ...
            Catch exx As Exception
                DataString = DataString & "The following exception occurred at SendEmail : " + exx.ToString() & vbNewLine
