...
#Region " Protected Variables "
    ...
    <CLSCompliant(False)> Protected _blnIsNotTransaction As Boolean = True
    <CLSCompliant(False)> Protected _cmd As SqlCommand
    <CLSCompliant(False)> Protected _conn As SqlConnection
    <CLSCompliant(False)> Protected _strConnStr As String
    <CLSCompliant(False)> Protected _cmdText As String
    <CLSCompliant(False)> Protected _cmdType As CommandType = CommandType.StoredProcedure
    <CLSCompliant(False)> Protected _sqlTransaction As SqlTransaction
    <CLSCompliant(False)> Protected _blnIsConnectionClosed As Boolean
    <CLSCompliant(False)> Protected _blnIsDistributedTransaction As Boolean
    <CLSCompliant(False)> Protected _intTimeout As Integer = 50 'Old value 30
...
#Region " Constructors() "
    Public Sub New(Optional ByVal CustomerID As Integer = 0, Optional ByVal connType As Integer = 0)
        ...
        ElseIf CustomerID = -1 Then
            _strConnStr = Connection.GetConnectionString(CustomerID, -1)

        Else
            '  If (connType = 0) Then
            ' _strConnStr = Connection.GetConnectionString(CustomerID, 0)
            ' Else
            _strConnStr = Connection.GetConnectionString(CustomerID, connType)
            ' End If
        End If
...
#Region " Public Properties "
    ...
    <CLSCompliant(False)> Public ReadOnly Property _Connection() As SqlConnection
        Get
            Return _conn
        End Get
    ...
    <CLSCompliant(False)> Public Property _Transaction() As SqlTransaction
        Get
